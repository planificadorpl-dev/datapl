export function renderAdminPanel(appState) {
  ...
  const planesDomList = appState.planes.filter(p => p.tipo === 'Domiciliario').map(p => `
    <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-[#E5E5EA] mb-2 shadow-sm ${!p.activo ? 'opacity-50' : ''}">
      <div class="flex flex-col">
        <span class="font-medium text-black">${p.nombre}${p.has_tv ? ' + TV' : ''}</span>
        <span class="text-[10px] uppercase text-[#8E8E93] font-bold">${p.tipo}</span>
      </div>
      <div class="flex items-center gap-2">
        <button class="btn-toggle-plan-tv p-2 rounded-lg transition-colors ${p.has_tv ? 'text-[#007AFF] bg-blue-50' : 'text-gray-400 hover:bg-gray-50'}" title="Toggle TV" data-id="${p.id}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path d="M2 6a2 2 0 012-2h12a2 2 0 012 2v7a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm2 4h12V6H4v4z" />
          </svg>
        </button>
        <button class="btn-toggle-plan-active p-2 rounded-lg transition-colors ${p.activo ? 'text-[#34C759] bg-green-50' : 'text-gray-400 bg-gray-50'}" title="Activo/Inactivo" data-id="${p.id}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
          </svg>
        </button>
        <button class="btn-delete-plan text-red-500 p-2 hover:bg-red-50 rounded-lg transition-colors" data-id="${p.id}">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
          </svg>
        </button>
  ...
  const planesEmpList = appState.planes.filter(p => p.tipo === 'Empresarial').map(p => `
    <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-[#E5E5EA] mb-2 shadow-sm ${!p.activo ? 'opacity-50' : ''}">
      <div class="flex flex-col">
        <span class="font-medium text-black">${p.nombre}${p.has_tv ? ' + TV' : ''}</span>
        <span class="text-[10px] uppercase text-[#8E8E93] font-bold">${p.tipo}</span>
